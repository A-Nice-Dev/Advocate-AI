...
def db_create_vault_user(email, name, password):
    ...
    try:
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?)
        ''', (email, name, hash_key(password), ts))
...
def db_verify_vault_access(email, password):
    ...
    conn = sqlite3.connect(SQL_DB_FILE)
    cursor = conn.cursor()
    cursor.execute("SELECT full_name FROM users WHERE email=? AND vault_key=?", (email, hash_key(password)))
    res = cursor.fetchone()
    conn.close()
    if res:
        ...
        for f in os.listdir(DATA_FOLDER):
            if f.lower().endswith(".pdf") and f not in indexed:
                try:
                    ...
                    cursor.execute(
                        "INSERT INTO law_assets (filename, filesize_kb, page_count, sync_timestamp) VALUES (?,?,?,?)",
                        (f, os.path.getsize(os.path.join(DATA_FOLDER, f)) / 1024, len(pdf.pages),
...
def render_chamber_workstation():
    ...
    with st.sidebar:
        ...
        with c2:
            if st.button("📧 Brief"):
                if dispatch_legal_brief_smtp(u_mail, st.session_state.current_chamber,
                                             db_fetch_chamber_history(u_mail, st.session_state.current_chamber)):
            ...
            with st.chat_message("assistant"):
                with st.spinner("Processing Strategy..."):
                    try:
                        ...
                        db_log_consultation(st.session_state.user_email, st.session_state.current_chamber, "assistant",
                                            response)
